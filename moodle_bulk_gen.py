...
def read_password_generator_api_key():
    try:
        ...
        print("Error: 'password_api_key.txt' file not found. File has been created. Please place your API key inside it.")
...
if not args.input or not args.output:
    ...
    exit(1)
elif not str(args.input).endswith(".csv") or not str(args.output).endswith(".csv"):
    print("Input and output files must be .csv format.")
    exit(1)

if args.local_api:
    password_web_api = False
else:
    password_web_api = True
...
outputWriter = csv.DictWriter(output_csv, fieldnames=students_list_fieldnames)
outputWriter.writeheader()

email_csv_name = str(args.output).replace(".csv", "_email.csv")
output_email_csv = open(email_csv_name, "w", newline="", encoding="utf-8")
output_email_Writer = csv.DictWriter(output_email_csv, fieldnames=email_fieldnames, delimiter=",")
...
output_cohorts_Writer = csv.DictWriter(output_cohorts_csv, fieldnames=cohort_fieldnames, delimiter=",")
...
duplicates_csv_Writer = csv.DictWriter(duplicates_csv, fieldnames=duplicates_fieldnames, delimiter=",")
duplicates_csv_Writer.writeheader()

print_separator()
print("A program for generating Moodle user accounts information for bulk uploading, emailing, and cohort lists.")
print_separator()
...
# Close all files
for file in [input_csv, output_csv, output_email_csv, output_cohorts_csv, duplicates_csv]:
    file.close()
